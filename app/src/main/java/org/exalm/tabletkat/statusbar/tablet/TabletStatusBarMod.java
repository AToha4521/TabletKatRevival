...
import org.exalm.tabletkat.statusbar.policy.BatteryMod;
...
public class TabletStatusBarMod extends BaseStatusBarMod implements
    ...
    private ContentObserver mUserSetupObserver = new ContentObserver(new Handler()) {
        ...
        public void onChange(boolean selfChange) {
            final boolean userSetup = 0 != (Integer) XposedHelpers.callStaticMethod(Settings.Secure.class, "getIntForUser",
...
//        lp.windowAnimations = XposedHelpers.getStaticIntField(TabletKatModule.mComAndroidInternalRStyleClass, "Animation"); // == no animation
        ...
        if (NOTIFICATION_PEEK_ENABLED) {
            ...
            lp.windowAnimations = XposedHelpers.getStaticIntField(TabletKatModule.mComAndroidInternalRStyleClass, "Animation_Toast");
...
//TODO:        lp.windowAnimations = R.style.Animation_RecentPanel;
    ...
    private void addHeadsUpView() {
        WindowManager.LayoutParams lp = new WindowManager.LayoutParams(
                ...
                2014, // WindowManager.LayoutParams.TYPE_STATUS_BAR_PANEL, // above the status bar!
                WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN
                        | WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS
...
//            Log.d(TAG, "size=" + newIconSize + " padding=" + newIconHPadding);
    ...
    protected View makeStatusBarView() {
        ...
        // Sync initial disabled state from the original status bar
        try {
            mDisabled = XposedHelpers.getIntField(self, "mDisabled");
            Log.i(TAG, "Syncing initial mDisabled: 0x" + Integer.toHexString(mDisabled));
        } catch (Throwable t) {
            // Field might be named differently or missing
        }
        ...
        View sbView = ViewHelper.replaceView(temp, new TabletStatusBarView(context, mBarService));
        if (!(sbView instanceof TabletStatusBarView)) {
            Log.e(TAG, "Failed to create TabletStatusBarView!");
            return null;
        }
        final TabletStatusBarView sb = (TabletStatusBarView) sbView;
        ...
        final NotificationTriggerTouchListener listener = new NotificationTriggerTouchListener();
        mNotificationArea.setOnTouchListener(listener);
        
        // Also apply specifically to clock and battery to ensure they don't swallow the touch
        View clock = sb.findViewById(SystemR.id.clock);
        if (clock != null) {
            clock.setClickable(true);
            clock.setOnTouchListener(listener);
        }
        
        View battery = sb.findViewById(SystemR.id.battery);
        if (battery != null) {
            battery.setClickable(true);
            battery.setOnTouchListener(listener);
        }

        if (mNotificationTrigger != null) {
            mNotificationTrigger.setClickable(true);
            mNotificationTrigger.setOnTouchListener(listener);
        }
        ...
        if (mNotificationIconArea != null) {
            mNotificationIconArea.setOnTouchListener(listener);
        }
        ...
        mInputMethodSwitchButton.setVisibility(View.GONE);
    ...
    public void setClockFont(TextView clock, boolean useOldFont){
        if (clock == null || getContext() == null) return;
    ...
    private void finalizeStatusBarView(TabletStatusBarView v) {
        if (v == null || mContext == null) return;
        ...
        ViewStub cluster = new ViewStub(mLargeIconContext != null ? mLargeIconContext : mContext);
        ViewHelper.replaceView(f.getChildAt(0), cluster);
        cluster.setLayoutResource(SystemR.layout.signal_cluster_view);
        try {
            cluster.inflate();
        ...
        View battery = ViewHelper.replaceView(v, SystemR.id.battery, (View) XposedHelpers.newInstance(TabletKatModule.mBatteryMeterViewClass, mContext));
        try {
        ...
        mSettingsReceiver = TabletKatModule.registerReceiver(mContext, new OnPreferenceChangedListener() {
            private void delayedUpdate() {
                mHandler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        updateTheme(TkPrefs.getThemeStyle());
                    }
                }, 200);
            }
            ...
            public void onPreferenceChanged(String key, boolean value) {
                if (key.equals(TkPrefs.KEY_CLOCK_FONT)) {
                    setClockFont(clock, value);
                }
                if (key.equals(TkPrefs.KEY_CLOCK_SECONDS)) {
                    org.exalm.tabletkat.statusbar.policy.ClockMod.refreshAllClocks();
                }
                if (key.equals(TkPrefs.KEY_BATTERY_PERCENT)){
                    percent.setVisibility(value ? View.VISIBLE : View.GONE);
                }
                if (key.equals(TkPrefs.KEY_HOLO_TINT)) {
                    delayedUpdate();
                }
                if (key.startsWith("navbutton_")) {
                    NavButtonsMod.updateButtonVisibility(v);
                    delayedUpdate();
                }
            }
            ...
            public void onPreferenceChanged(String key, String value) {
                if (key.equals("theme_style") || key.equals(TkPrefs.KEY_STATUS_ICON_STYLE)) {
                    delayedUpdate();
                }
            }
            ...
            public void init(XSharedPreferences pref) {
                setClockFont(clock, pref.getBoolean(TkPrefs.KEY_CLOCK_FONT, false));
                org.exalm.tabletkat.statusbar.policy.ClockMod.refreshAllClocks();
                percent.setVisibility(pref.getBoolean(TkPrefs.KEY_BATTERY_PERCENT, false) ? View.VISIBLE : View.GONE);
                NavButtonsMod.updateButtonVisibility(v);
                updateTheme(TkPrefs.getThemeStyle());
            }
        });
    ...
    private void updateTheme(String theme) {
        if (mStatusBarView == null) {
            Log.w(TAG, "updateTheme: mStatusBarView is null, cannot update");
            return;
        }
        boolean isHolo = TkPrefs.isHoloTheme();
        boolean isHoloTint = TkPrefs.isHoloTintEnabled();
        
        Log.i(TAG, "updateTheme: theme=" + theme + ", isHolo=" + isHolo + ", isHoloTint=" + isHoloTint);

        // Clock gets the Holo blue if enabled
        int clockTint = isHolo ? 0xFF33b5e5 : 0xFFFFFFFF;
        // Navigation buttons should stay white/grey even in ICS as requested
        int navTint = 0xFFFFFFFF;

        // Apply holographic tint if enabled
        if (isHoloTint) {
            int opaque = 0xFF1A1F2E; // Dark navy-blue
            int semi = 0x7F1A1F2E;
            mStatusBarView.getBarTransitions().setColors(opaque, semi);
        } else {
            Resources res = mContext.getResources();
            int opaque = res.getColor(SystemR.color.system_bar_background_opaque);
            int semi = res.getColor(SystemR.color.system_bar_background_semi_transparent);
            mStatusBarView.getBarTransitions().setColors(opaque, semi);
        }
        mStatusBarView.invalidate(); // Force redraw of the bar background

        // Tint the main nav buttons
        ViewGroup navButtons = (ViewGroup) mStatusBarView.findViewById(SystemR.id.nav_buttons);
        if (navButtons != null) {
            for (int i = 0; i < navButtons.getChildCount(); i++) {
                View child = navButtons.getChildAt(i);
                if (child instanceof ImageView) {
                    ((ImageView) child).setColorFilter(navTint, android.graphics.PorterDuff.Mode.SRC_ATOP);
                }
            }
        }

        // Update extra nav buttons if they exist
        View extraButtons = mStatusBarView.findViewWithTag("tk_extra_buttons");
        if (extraButtons instanceof ViewGroup) {
            ViewGroup vg = (ViewGroup) extraButtons;
        ...
        // Tint the clock
        TextView clock = (TextView) mStatusBarView.findViewById(SystemR.id.clock);
        if (clock != null) {
            clock.setTextColor(clockTint);
        }

        // Tint the battery percentage text
        TextView batteryText = (TextView) mStatusBarView.findViewById(TkR.id.battery_text);
        if (batteryText != null) {
            batteryText.setTextColor(clockTint);
        }

        // Tint the battery icon
        View battery = mStatusBarView.findViewById(SystemR.id.battery);
        if (battery != null) {
            if (isHolo) {
                // We use SRC_ATOP to tint the white parts of the battery
                // If it's a custom view drawing with paint, we might need a different approach,
                // but setColorFilter often works on the layer/view level in KK.
                try {
                    // Try to set color filter directly
                    XposedHelpers.callMethod(battery, "setColorFilter", clockTint, android.graphics.PorterDuff.Mode.SRC_ATOP);
                    // Also try on the drawable itself if it has one
                    Drawable d = battery.getBackground();
                    if (d != null) {
                        d.setColorFilter(clockTint, android.graphics.PorterDuff.Mode.SRC_ATOP);
                    }
                } catch (Throwable t) {
                    // Fallback for some BatteryMeterView versions
                    try {
                        XposedHelpers.callMethod(battery, "setLayerType", View.LAYER_TYPE_SOFTWARE, null);
                        battery.invalidate();
                    } catch (Throwable ignored) {}
                }
            } else {
                try {
                    XposedHelpers.callMethod(battery, "setColorFilter", null);
                } catch (Throwable ignored) {}
            }
        }

        // Tint system status icons (Bluetooth, etc.)
        if (mStatusIcons != null) {
            for (int i = 0; i < mStatusIcons.getChildCount(); i++) {
                View child = mStatusIcons.getChildAt(i);
                if (child instanceof ImageView) {
                    if (isHolo) {
                        ((ImageView) child).setColorFilter(clockTint, android.graphics.PorterDuff.Mode.SRC_ATOP);
                    } else {
        ...
        // Tint Signal Cluster icons
        SignalClusterMod.refreshAll();

        // Refresh battery
        BatteryMod.refreshAll();

        // Refresh clock
        org.exalm.tabletkat.statusbar.policy.ClockMod.refreshAllClocks();

        // Tint components in the notification panel if it exists
        if (mNotificationPanel != null) {
            TextView panelClock = (TextView) mNotificationPanel.findViewById(SystemR.id.clock);
            if (panelClock != null) {
                panelClock.setTextColor(clockTint);
            }
            TextView panelBatteryText = (TextView) mNotificationPanel.findViewById(TkR.id.battery_text);
            if (panelBatteryText != null) {
                panelBatteryText.setTextColor(clockTint);
            }
            // Tint icons in the notification panel title signal cluster
            View panelCluster = mNotificationPanel.findViewById(SystemR.id.signal_cluster);
            if (panelCluster instanceof ViewGroup) {
                tintRecursive((ViewGroup) panelCluster, isHolo ? clockTint : 0);
            }

            // Tint the battery icon in notification panel
            View panelBattery = mNotificationPanel.findViewById(SystemR.id.battery);
            if (panelBattery != null) {
                if (isHolo) {
                    try {
                        XposedHelpers.callMethod(panelBattery, "setColorFilter", clockTint, android.graphics.PorterDuff.Mode.SRC_ATOP);
                        Drawable d = panelBattery.getBackground();
                        if (d != null) {
                            d.setColorFilter(clockTint, android.graphics.PorterDuff.Mode.SRC_ATOP);
                        }
                    } catch (Throwable ignored) {}
                    ...
                    try {
                        XposedHelpers.callMethod(panelBattery, "setColorFilter", null);
                    } catch (Throwable ignored) {}
                }
            }
        }
    ...
    private void tintRecursive(ViewGroup parent, int color) {
        for (int i = 0; i < parent.getChildCount(); i++) {
            View child = parent.getChildAt(i);
            if (child instanceof ImageView) {
                if (color != 0) {
                    ((ImageView) child).setColorFilter(color, android.graphics.PorterDuff.Mode.SRC_ATOP);
                } else {
                    ((ImageView) child).setColorFilter(null);
                }
...
//                              : mNotificationPeekScrubRight);
            ...
            case MSG_OPEN_NOTIFICATION_PANEL:
                if (DEBUG || true) Log.d(TAG, "opening notifications panel; mNotificationPanel=" + mNotificationPanel);
                awakenDreams();
                if (mNotificationPanel != null) {
                    // Force show even if it thinks it's already showing, just in case state got out of sync
                    ...
                    Log.e(TAG, "mNotificationPanel is null, cannot open!");
                }
                break;
...
//FIXME
    ...
    private class NotificationTriggerTouchListener implements View.OnTouchListener {
        ...
        public boolean onTouch(View v, MotionEvent event) {
            // Toast for debugging
            // if (event.getAction() == MotionEvent.ACTION_DOWN) Toast.makeText(mContext, "StatusBar Touch!", Toast.LENGTH_SHORT).show();

            if (DEBUG || true) Log.d(TAG, "Touch on notification area: " + event.getAction() + " mDisabled=0x" + Integer.toHexString(mDisabled));

            final int action = event.getAction();

            // Safety: if expansion is disabled but user is tapping, maybe force clear it after a long press?
            // For now, let's just log and see if it's really the cause.
            
            if ((mDisabled & StatusBarManager.DISABLE_EXPAND) != 0) {
                // If it's a tap, maybe we should allow it if we are already provisioned?
                if (isDeviceProvisioned()) {
                     Log.w(TAG, "Expansion is DISABLED but device is PROVISIONED. Tapping anyway.");
                     // return true; // Still blocking for now to confirm with logs
                }
                if (DEBUG || true) Log.d(TAG, "Touch blocked by DISABLE_EXPAND");
                return true;
            }
            ...
            switch (action) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (mVT != null) {
                        ...
                        // 1. Check for fling (fast swipe up)
                        // 2. Check for drag (slow swipe up by more than 2x touch slop)
                        if (mVT.getYVelocity() < -mNotificationFlingVelocity || diffY < -mTouchSlop * 2) {
                ...
                case MotionEvent.ACTION_CANCEL:
                    ...
                    if (mVT != null) {
                        if (action == MotionEvent.ACTION_UP
                                // was this a sloppy tap? (standard slop is fine)
                                && Math.abs(event.getX() - mInitialTouchX) < mTouchSlop * 2
                                && Math.abs(event.getY() - mInitialTouchY) < mTouchSlop * 2
    ...
    private void resetUserSetupObserver() {
        ...
        XposedHelpers.callMethod(mContext.getContentResolver(), "registerContentObserver",
        ...
        try {
            XposedHelpers.findAndHookMethod(base, "updateSearchPanel", new XC_MethodReplacement() {
                ...
                protected Object replaceHookedMethod(MethodHookParam methodHookParam) throws Throwable {
                    ...
                    if (self == null || mContext == null) {
                        Log.e(TAG, "self or mContext is null in updateSearchPanel!");
                        return null;
                    }
                    ...
                    Context c2 = mContext.createConfigurationContext(config);
                    
                    int layoutId = SystemR.layout.status_bar_search_panel;
                    if (layoutId <= 0) {
                        // Fallback: try to find it manually if SystemR isn't init yet
                        layoutId = mContext.getResources().getIdentifier("status_bar_search_panel", "layout", "com.android.systemui");
                    }
                    if (layoutId <= 0) {
                        Log.e(TAG, "Search panel layout not found!");
                        return null;
                    }
                    
                    if (c2 == null) c2 = mContext;
                    
                    mSearchPanelView = LayoutInflater.from(c2).inflate(layoutId, tmpRoot, false);
                    if (mSearchPanelView == null) return null;
                    
                    mSearchPanelView.setOnTouchListener(
                            new TouchOutsideListener(MSG_CLOSE_SEARCH_PANEL, mSearchPanelView));
                    mSearchPanelView.setVisibility(View.GONE);

                    ViewGroup.LayoutParams viewLp = mSearchPanelView.getLayoutParams();
                    if (viewLp == null) {
                        viewLp = new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                    }
                    
                    WindowManager.LayoutParams lp = getSearchLayoutParams(viewLp);
...
//                Toast.makeText(mContext, "removeIcon(" + slot + ")", Toast.LENGTH_SHORT).show();
    ...
    public void onStart() {
        ...
        // Safety: clear DISABLE_EXPAND on start to ensure the panel works after reboot
        // System will set it again if it really needs to be disabled (e.g. during screen recording or setup)
        mDisabled &= ~StatusBarManager.DISABLE_EXPAND;
        Log.i(TAG, "onStart: force cleared DISABLE_EXPAND. current mDisabled=0x" + Integer.toHexString(mDisabled));
                ...
                case MotionEvent.ACTION_CANCEL:
                    ...
                    if (!peeking) {
                        if (action == MotionEvent.ACTION_UP
                                ...
                                && Math.abs(event.getY() - mInitialTouchY) < mTouchSlop
                                // dragging off the bottom doesn't count
                                && (int)event.getY() < v.getBottom()) {
                            Message peekMsg = mHandler.obtainMessage(MSG_OPEN_NOTIFICATION_PEEK);
                            peekMsg.arg1 = mPeekIndex;
